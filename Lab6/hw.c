...
double calculate_mse(double exact, double approximate);
double calculate_rmse(double mse);
double calculate_mae(double exact, double approximate);
double calculate_mape(double exact, double approximate);

int main() 
{
    double x =0.0, h = 0.01, y = 1.0;
    ...
    double MAE = calculate_mae(Y_exact, Y_euler);
    
    printf("h\t\tY_euler\t\tY_rungekutta_\tY_exact\t\tPer_err_Euler\tPer_err_RK\n");
    printf("%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf",h,Y_euler, Y_kutta, Y_exact, Perr_err_EU, Perr_err_RK + 1);
